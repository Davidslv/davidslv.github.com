/* castplay.js — a tiny, self-contained asciinema-cast player.
   ...
   data-cast may be EITHER a URL (fetched — needs HTTP) OR a "#id" pointing at an inline
   <script type="text/cast" id="..."> block holding the raw .cast text. Prefer the inline
   form so the deck is truly self-contained and the demo animates even when the file is
   opened directly from disk (file://), where fetch() is blocked by the browser. */
(function () {
  /* tuned for the davidslv.uk dark terminal surface (--term-bg #0d1117) */
  var COL = { 32:'#3fb968', 90:'#8b949e', 37:'#c9d1d9', 92:'#56d364',
              91:'#ff6a5a', 33:'#e3b341', 36:'#58a6ff', 97:'#f0f6fc' };
  ...
  Player.prototype.parse=function(txt, cb){
    try { this.events=txt.split('\n').filter(Boolean).slice(1).map(function(l){return JSON.parse(l);}); cb(); }
  ...
  Player.prototype.load=function(cb){
    ...
    if(src.charAt(0)==='#'){                       // inline: read from a <script type="text/cast"> block
      var node=document.querySelector(src);
      if(node){ self.parse(node.textContent, cb); } else { self.el.textContent='(cast not found: '+src+')'; }
      return;
    }
    fetch(src).then(function(r){return r.text();}).then(function(txt){ self.parse(txt, cb); })
      .catch(function(){ self.el.textContent='(cast failed to load — serve over http, or inline it)'; });
  };
